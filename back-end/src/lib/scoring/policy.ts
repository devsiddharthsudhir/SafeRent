...
function textOf(listing: Listing): string {
...
function findFirstAmountNear(text: string, keywordRe: RegExp): number | null {
...
export function extractPolicySignals(listing: Listing, weightMap: Record<string, number>): Signal[] {
  ...
  if (province === "BC") {
    ...
    if (mentions(text, /(application\s*fee|processing\s*fee|admin\s*fee)/i)) {
      ...
      out.push({
        ...
        why_it_matters:
          "In BC, application or processing fees are a common predatory pattern and may violate standard tenancy expectations.",
    ...
    const deposit =
      findFirstAmountNear(raw, /(damage\s*deposit|security\s*deposit|deposit)/i) ?? null;
    if (deposit && listing.price && deposit > listing.price * 0.5 + 5) {
      ...
      out.push({
        ...
        why_it_matters:
          "In BC, unusually high deposits are often a sign of a predatory listing or misinformation about tenancy rules.",
    ...
    if (mentions(text, /(monthly\s*pet\s*fee|pet\s*fee\s*per\s*month|\$\s*\d+\s*pet\s*fee)/i)) {
      ...
      out.push({
        ...
        why_it_matters:
          "Ongoing pet fees are often used to inflate effective rent and can be a predatory term.",
  ...
  if (province === "ON") {
    ...
    if (mentions(text, /(application\s*fee|processing\s*fee|admin\s*fee)/i)) {
      ...
      out.push({
        ...
        why_it_matters:
          "In Ontario, upfront application or processing fees are a common predatory pattern.",
    ...
    if (mentions(text, /(security\s*deposit|damage\s*deposit|pet\s*deposit)/i)) {
      ...
      out.push({
        ...
        why_it_matters:
          "Ontario leases usually use a last-month rent deposit. Extra deposit types can be a predatory sign.",
  ...
  if (mentions(text, /(non\s*-?refundable|not\s*refundable|no\s*refund)/i)) {
    ...
    out.push({
      ...
      why_it_matters:
        "Non-refundable deposits/fees are a common predatory term and reduce your ability to dispute abuse.",
  ...
  if (mentions(text, /(enter|access)\s*(the\s*)?(unit|premises).*(any\s*time|at\s*any\s*time|without\s*notice)/i)) {
    ...
    out.push({
      ...
      why_it_matters:
        "Clauses allowing entry at any time can be abusive and may conflict with provincial tenancy rules.",
  ...
  if (mentions(text, /(rent|rate).*(increase|raise).*(any\s*time|at\s*any\s*time|without\s*notice)/i)) {
    ...
    out.push({
      ...
      why_it_matters:
        "Rent increase rules are regulated. Clauses allowing 'any time' increases are often predatory.",
  ...
  if (mentions(text, /(late\s*fee|penalty).*(per\s*day|daily)/i)) {
    ...
    out.push({
      ...
      why_it_matters:
        "Daily penalties can be used to trap tenants in debt and are a common abusive pattern.",
  ...
  if (mentions(text, /(waive|waiver)\s*(any\s*)?(rights|claims)|hold\s*harmless|release\s*(the\s*)?landlord|indemnify\s*(the\s*)?landlord/i)) {
    ...
    out.push({
      ...
      why_it_matters:
        "Broad waivers can remove important tenant protections and are often red flags.",
