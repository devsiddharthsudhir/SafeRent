...
export function extractBehaviorSignals(listing: Listing, weightMap: Record<string, number>): Signal[] {
  const out: Signal[] = [];
  ...
  if (descLen > 0 && descLen < 80) {
    ...
    out.push({
      ...
      why_it_matters:
        "Thin details make it harder to verify legitimacy and can be a sign of low-effort scam postings.",
  ...
  if (phoneRe.test(text) || emailRe.test(text)) {
    ...
    out.push({
      ...
      why_it_matters:
        "Moving off-platform reduces accountability and moderation, and is often used in deposit scams.",
  ...
  const addrHint = (listing as any).address_hint || (listing as any).addressHint;
  if (!listing.city && !listing.neighborhood && !addrHint) {
    ...
    out.push({
      ...
      why_it_matters:
        "Listings that hide city/neighborhood are harder to verify and sometimes used for bait pricing.",
