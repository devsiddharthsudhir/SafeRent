...
function edgeEnergy(img: Jimp): number {
...
export async function extractImageSignals(
  imageBuffers: Buffer[],
  knownHashes: string[],
  weightMap: Record<string, number>
): Promise<{ signals: Signal[]; hashes: string[] }> {
  ...
  for (const buf of imageBuffers) {
    try {
      ...
      let minD = Infinity;
      for (const h of knownHashes) minD = Math.min(minD, hamming(hash, h));
