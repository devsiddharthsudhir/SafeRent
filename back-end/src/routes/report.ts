...
function verdictText(v: string): string {
  ...
  if (v === "likely_scam") return "High risk indicators";
  if (v === "likely_predatory") return "Higher risk indicators";
  if (v === "likely_legit") return "Fewer risk indicators";
...
function fmtMoney(currency: string | undefined, price: any): string {
  const cur = (currency || "CAD").toUpperCase();
  const n = Number(price);
  if (!Number.isFinite(n)) return `${cur} ${safeStr(price)}`.trim();
  try {
...
function asBullets(items: any[], fallback: string) {
...
reportRouter.get("/report/:analysisId", (req, res) => {
  ...
  signals.sort((s1: any, s2: any) => Math.abs(Number(s2?.contribution || 0)) - Math.abs(Number(s1?.contribution || 0)));
  ...
  const md = [
    ...
    ...(Array.isArray(a.crossposts) && a.crossposts.length
      ? [
          ...
          ...a.crossposts.slice(0, 6).map((m: any) => {
