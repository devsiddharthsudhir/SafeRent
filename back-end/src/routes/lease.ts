...
leaseRouter.post("/lease/simplify", upload.single("lease"), async (req, res) => {
  try {
    const f = (req.file as Express.Multer.File) || null;
    if (!f) return res.status(400).json({ error: "Missing file: field name must be 'lease'" });

    const provinceHint = String((req.body as any)?.province || "").trim();
