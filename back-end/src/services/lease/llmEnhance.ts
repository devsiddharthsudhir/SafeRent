import type { LeaseFlag, LeaseSimplifyResult } from "./simplifyLease.js";
...
function redactLeaseText(text: string): string {
  ...
  // Postal codes (Canada)
  t = t.replace(
    /\b[ABCEGHJ-NPRSTVXY]\d[ABCEGHJ-NPRSTV-Z][\s-]?\d[ABCEGHJ-NPRSTV-Z]\d\b/gi,
    "[REDACTED_POSTAL]"
  );
  // Very rough street address lines
  t = t.replace(
    /\b\d{1,6}\s+[A-Za-z0-9.\- ]{2,40}\s+(Street|St|Avenue|Ave|Road|Rd|Boulevard|Blvd|Drive|Dr|Lane|Ln|Crescent|Cres|Way|Place|Pl)\b/gi,
    "[REDACTED_ADDRESS]"
  );
  return t;
}
 ...
 */
function computeLeaseRiskScoreFromFlags(flags: LeaseFlag[], extractedChars = 0): number {
...
function leaseVerdictFromScore(score: number, extractedChars = 0): "low" | "medium" | "high" | "unclear" {
  const lowText = extractedChars < 700;
  if (lowText) return "unclear";
...
type LlmOut = {
  laymanSummaryExtra?: string[];
  flagsExtra?: LlmFlag[];
};

function safeJsonExtract(input: string): any {
...
export async function maybeEnhanceLeaseWithLLM(
  base: LeaseSimplifyResult,
  fullText: string
): Promise<LeaseSimplifyResult & { llmUsed?: boolean }> {
  ...
  const system =
    `You are a careful assistant helping a tenant understand a Canadian residential lease.\n\n` +
  ...
  const user = {
    ...
    want: {
      ...
      flagsExtra:
        "Add up to 6 extra flags for missed edge cases. Use severity low/medium/high. Include an excerpt if you can find it in the text.",
    },
  };
  ...
  try {
    ...
    const flagsExtra: LeaseFlag[] = [];
    for (const f of flagsExtraRaw) {
      if (!f) continue;
      const sev = String((f as any).severity || "").toLowerCase();
      if (!["low", "medium", "high"].includes(sev)) continue;

      const title = String((f as any).title || "").trim();
      const why = String((f as any).why || "").trim();
      const suggestion = String((f as any).suggestion || "").trim();
      if (!title || !why || !suggestion) continue;

      const excerpt = String((f as any).excerpt || "").trim() || undefined;
    ...
    const merged: LeaseSimplifyResult & { llmUsed?: boolean } = {
      ...base,
...
function dedupeFlags(flags: LeaseFlag[]): LeaseFlag[] {
  const sevRank: Record<string, number> = { high: 3, medium: 2, low: 1 };
  const byKey = new Map<string, LeaseFlag>();
  ...
  for (const f of flags) {
    ...
    if ((sevRank[f.severity] || 0) > (sevRank[prev.severity] || 0)) byKey.set(k, f);
  }
