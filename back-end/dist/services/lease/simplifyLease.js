...
function looksLikeClauseHeading(line) {
    ...
    if (!l)
        return false;
    if (l.length < 4 || l.length > 120)
        return false;
    if (/^page\s+\d+/i.test(l))
        return false;
    if (/^(the\s+)?residential\s+tenancy/i.test(l))
        return false;
    // Numbered clauses: "1.", "1.1", "12.3" ...
    if (/^\d+(?:\.\d+){0,3}\s*[.)-]\s+\S/.test(l))
        return true;
    // Section/article style
    if (/^(section|article)\s+\d+\b/i.test(l))
        return true;
    // Schedule/appendix headers
    if (/^(schedule|appendix)\s+[A-Z0-9]+\b/i.test(l))
        return true;
    // ALL CAPS headings
    if (isMostlyUpper(l) && l.split(/\s+/).length <= 10)
        return true;
    return false;
}
function normalizeHeadingToTitle(line) {
...
function splitLeaseIntoClauses(fullText) {
...
function plainEnglishForClause(raw) {
    ...
    if (/deposit|damage deposit|security deposit/.test(lower)) {
        const a = Number.isFinite(amount) ? `about $${Math.round(amount)}` : "a deposit";
    ...
    if (/rent/.test(lower) && (/(due|payable|payment)/.test(lower) || amount)) {
        const a = Number.isFinite(amount) ? `around $${Math.round(amount)}` : "the stated amount";
    ...
    if (/terminate|termination|end this lease|notice/.test(lower)) {
        const d = Number.isFinite(dayCount) ? `${dayCount} days` : "a certain number of days";
...
function extractSnippet(text, re, maxLen = 220) {
...
function findDaysNear(text, keyword) {
    const m = text.match(new RegExp(`${keyword.source}[^\n]{0,90}`, "i"));
    if (!m)
        return undefined;
...
function guessProvinceFromLeaseText(text) {
    const t = text.toLowerCase();
    if (/\b(british columbia|\bbc\b)\b/.test(t))
        return "BC";
    if (/\b(ontario|\bon\b)\b/.test(t))
        return "ON";
    if (/\b(alberta|\bab\b)\b/.test(t))
        return "AB";
    if (/\b(quebec|québec|\bqc\b)\b/.test(t))
        return "QC";
    return "";
}
...
function scoreLease(flags, extractedChars, warnings) {
...
function buildFlags(text, province) {
    const flags = [];
    ...
    for (const pat of lib) {
        if (pat.provinces && pat.provinces.length) {
            if (!pat.provinces.map(x => x.toUpperCase()).includes(p))
                continue;
        }
        if (pat.re.test(text)) {
            add({
...
function scoreLeaseFromFlags(flags, extractedChars, warnings) {
    const sevRank = { high: 3, medium: 2, low: 1 };
    ...
    const verdict = lowText ? "unclear" : score >= 70 ? "high" : score >= 35 ? "medium" : "low";
    return { score, verdict, counts, sevRank };
}
export async function extractLeaseTextFromUpload(file) {
    const warnings = [];
    ...
    if (type.includes("pdf") || name.endsWith(".pdf")) {
        ...
        if (parsed) {
            ...
            if (text.length < 800)
                warnings.push("Text extraction looks short. If this is a scanned lease, try uploading a selectable-text PDF or paste the lease text.");
    ...
    if (text.length < 400)
        warnings.push("Lease text is short. For best results, paste the full lease including addendums.");
    return { text, warnings };
}
export function simplifyLeaseText(text, provinceHint) {
    const cleaned = cleanText(text);
    const warnings = [];
    if (!cleaned)
        warnings.push("No text detected.");
    ...
    if (/(month\s*to\s*month)/i.test(cleaned))
        termType = "month_to_month";
    if (/(fixed\s*term|term\s*of\s*this\s*lease|ending\s*on)/i.test(cleaned))
        termType = "fixed";
    ...
    if (rentMonthly)
        laymanSummary.push(`Monthly rent looks like about CAD $${Math.round(rentMonthly)} (verify in the rent section).`);
    if (deposit)
        laymanSummary.push(`Deposit mentioned: about CAD $${Math.round(deposit)} (confirm if refundable and what it's for).`);
    if (termType === "month_to_month")
        laymanSummary.push("The lease appears to be month-to-month (more flexible, but notice rules matter).");
    if (termType === "fixed")
        laymanSummary.push("The lease appears to be a fixed term (check start/end dates and renewal rules).");
    if (noticeToEndDays)
        laymanSummary.push(`Notice to end/terminate is mentioned around ${noticeToEndDays} days (confirm the exact clause).`);
    if (utilities)
        laymanSummary.push(`Utilities clause spotted: “${utilities}”.`);
    if (occupants)
        laymanSummary.push(`Occupants clause spotted: “${occupants}”.`);
    if (cleaned.length < 800)
        warnings.push("Low-text lease detected. For best accuracy, paste the lease text (including addendums) or upload a selectable-text PDF.");
    ...
    const sevRank = { high: 3, medium: 2, low: 1 };
    const clauses = chunks.map((c, idx) => {
        const raw = c.raw.trim();
        const matched = [];
        let maxSev;
        for (const pat of clauseLib) {
            if (pat.provinces && pat.provinces.length) {
                if (!pat.provinces.map((x) => x.toUpperCase()).includes(String(provinceGuess || "").toUpperCase()))
                    continue;
            }
            if (pat.re.test(raw)) {
                matched.push(pat.id);
                if (!maxSev || sevRank[pat.severity] > sevRank[maxSev])
                    maxSev = pat.severity;
            }
        }
...
export function inferProvinceFromListingLike(input) {
    const listing = {
        title: input.title || "",
