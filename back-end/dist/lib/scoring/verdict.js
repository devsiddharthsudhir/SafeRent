...
const PREDATORY_SIGNAL_IDS = new Set([
    "nlp_application_fee",
    "price_anomaly_high",
...
function hasAny(signals, ids) {
    return signals.some((s) => ids.has(s.id));
}
export function verdictFromScore(score, signals) {
    ...
    if (score >= 78 && strong)
        return "likely_scam";
    if (score >= 78 && !strong)
        return predatory ? "likely_predatory" : "unclear";
    if (score >= 55)
        return predatory ? "likely_predatory" : "unclear";
    if (score <= 20)
        return "likely_legit";
...
export function recommendedActions(verdict) {
