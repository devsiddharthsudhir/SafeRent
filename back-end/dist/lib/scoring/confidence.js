...
function hasText(s, minLen) {
    return typeof s === "string" && s.trim().length >= minLen;
}
function guessHasImages(listing, opts) {
 ...
 */
export function computeConfidenceInfo(listing, signals, opts) {
    // --- 1) Data completeness
    const missing = [];
    let have = 0;
    let total = 0;
    const check = (ok, hintIfMissing) => {
        total += 1;
        if (ok)
            have += 1;
        else
            missing.push(hintIfMissing);
    };
    ...
    if (!hasText(listing.description, 60))
    ...
    const confidenceLabel = confidence >= 0.72 ? "high" : confidence >= 0.5 ? "medium" : "low";
    const dataQualityHints = missing.slice(0, 6);
    const redundancySteps = [
    ...
    if (confidenceLabel === "low") {
        redundancySteps.unshift("Result confidence is low: add missing details (URL, photos, full description) and re-check.");
    }
