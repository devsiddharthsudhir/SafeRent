...
function buildKnownHashes() {
    ...
    for (const a of analyses)
        if (Array.isArray(a.imageHashes))
            hashes.push(...a.imageHashes);
...
analyzeRouter.post("/analyze", upload.array("images", 6), async (req, res) => {
    try {
        ...
        if (req.is("application/json")) {
            const body = req.body;
            if (body?.listingId)
                listing = getDemoListings().find((l) => l.id === body.listingId) || null;
            else if (body?.listing)
                listing = body.listing;
        }
        else {
            const listingStr = req.body.listing;
            if (listingStr)
                listing = JSON.parse(listingStr);
            const listingId = req.body.listingId;
            if (!listing && listingId)
                listing = getDemoListings().find((l) => l.id === listingId) || null;
        }
        if (!listing)
            return res.status(400).json({ error: "Provide listingId or listing" });
        // Normalize field names so scoring/enrichment see consistent data
        listing = normalizeListingInput(listing);
        const files = req.files || [];
        const buffers = [];
        if (files.length === 0 && Array.isArray(listing.image_urls)) {
            for (const u of listing.image_urls) {
                const local = path.resolve(process.cwd(), "public", u.replace(/^\//, ""));
                if (fs.existsSync(local))
                    buffers.push(fs.readFileSync(local));
            }
        }
        else {
        ...
        const mode = base.riskProbability >= high ? "both" : base.riskProbability >= low ? "serper" : undefined;
        ...
        const analysis = {
            ...base,
            enrichmentStatus: shouldVerify ? "queued" : "skipped",
        ...
        if (shouldVerify)
            queueEnrichment(analysis.analysisId);
        res.json(analysis);
    }
    catch (e) {
...
analyzeRouter.post("/verify", express.json(), (req, res) => {
    ...
    const hasAnyProvider = paid || ddg;
    if (!webEnabled) {
        const patched = { ...a, enrichmentStatus: "skipped", enrichmentReason: "WEB_VERIFY_ENABLED=false" };
