...
adminRouter.post("/retention/sweep", express.json(), (req, res) => {
    try {
        ...
        const cacheRetentionDays = Number(req.body?.cacheRetentionDays);
        const uploadsRetentionDays = Number(req.body?.uploadsRetentionDays);
...
adminRouter.get("/metrics/latest", (_req, res) => {
    try {
        ...
        if (!fs.existsSync(p))
            return res.status(404).json({ error: "No metrics yet" });
