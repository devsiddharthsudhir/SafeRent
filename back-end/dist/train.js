...
function parseArgs(argv) {
    const out = { calibrate: true, report: true };
    for (const a of argv) {
        if (a === "--calibrate")
            out.calibrate = true;
        if (a === "--no-calibrate")
...
function computeZFromSignals(signals, weights) {
...
function fitPlattScaling(zs, ys, initA = 1, initB = 0) {
    ...
    for (let iter = 0; iter < 50; iter++) {
        ...
        a -= clamp(dA, -2, 2);
        b -= clamp(dB, -2, 2);
        if (Math.abs(dA) < 1e-4 && Math.abs(dB) < 1e-4)
            break;
    }
    // sane bounds
    a = clamp(a, 0.2, 6);
